...
9. From the project root, run `node scripts/validate-lab-journal.mjs`. Open the index and plate directly from
   disk. Check JavaScript-off readability, narrow layout, and print when adding a
   visual form.
10. Sign only after all unknown witness fields are explicitly marked.
...
does and does not change. Update chronological and thematic index lenses. Run the
static validator and report created files, evidence forms, reconstruction gaps,
open questions, and witness status.
```
